...
export function SettingsPanel(props: SettingsPanelProps) {
  ...
  const estimateRows = createMemo(() => {
    ...
    return [
      maybeDisplayRow("Estimated output", maybeFormatDuration(plan.estimatedOutputDurationSeconds)),
      maybeDisplayRow("Frames", formatPositiveInteger(plan.frameCount)),
      maybeDisplayRow("Effective speed", maybeFormatSpeed(plan.maybeEffectiveSpeed)),
    ].filter((maybeRow): maybeRow is DisplayRow => maybeRow !== null);
  });
  ...
  return (
    <section class={styles.panel} aria-labelledby="settings-title">
      ...
      {props.maybeMetadata ? (
        ...
        <p class={styles.emptyTip}>Choose a video to enable timelapse settings.</p>
      )}
    </section>
  );
}
...
function maybeDisplayRow(label: string, maybeValue: string | null): DisplayRow | null {
  if (!maybeValue) {
    return null;
  }
...
function formatPositiveInteger(value: number): string | null {
...
function maybeFormatSpeed(maybeSpeed: number | null | undefined): string | null {
  if (!maybeSpeed || !Number.isFinite(maybeSpeed)) {
    return null;
  }

  return `${maybeSpeed.toFixed(1)}x`;
}
